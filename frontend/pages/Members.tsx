...
export const Members: React.FC = () => {
  ...
  const handleExport = () => {
    // 1. Define headers
    const headers = ["ID", "Name", "Phone", "Join Date", "End Date", "Created By", "Status"];
    ...
    const csvRows = [
      ...
      ...filteredMembers.map(member => {
        return [
          member.id,
          `"${member.name}"`, // Quote strings to handle commas
          `"${member.phone}"`,
          member.joinDate,
          member.endDate || '',
          `"${member.createdBy}"`,
    ...
    // 4. Create a link and trigger download
            ...
            <div className="flex flex-wrap gap-3 w-full xl:w-auto">
              {(searchTerm || statusFilter !== 'all' || dateFilter || creatorFilter) && (
               ...
               <button 
                ...
                className="flex-1 xl:flex-none bg-[#111] border border-panda-border text-gray-300 px-6 py-2.5 rounded-xl font-medium hover:bg-white/5 hover:text-white transition-all flex items-center justify-center gap-2 h-[46px] mt-auto whitespace-nowrap"
              >
